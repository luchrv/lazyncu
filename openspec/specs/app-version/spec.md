...
## Purpose
Give the lazyncu binary reliable version identity: resolve version, commit, and build date across every build path (ldflags, `go install`, bare source builds) and expose them through a `--version` CLI flag that always works.
## Requirements
